...
// A gib is deliberately thin, so its support face is narrow and its patch is worth
// little; the plate is the shape a patch is actually for.
static auto make_spinner = make_spinning_box;
...
static void run_depth(int decoys, bool oblique) {
	...
	// --- with the contact patch armed -------------------------------------
	// Same traces, on a body that spins and with a patch buffer attached. The delta
	// against the two above is the whole cost of the manifold: one point_contents
	// descent per corner kept, plus the transform of each.
	auto plate = make_spinner(0.2, 0.05, 0.2);

	snprintf(name, sizeof(name), "  patch (discovery margin, 4 corners)");
	hop::bench::go(name, 200000, [&] {
		c.reset(); c.time = hop::scalar_traits<T>::one();
		seg.set_start_dir(vec(0, 0.3, 0), fall);
		t->trace_solid(c, plate.get(), V {}, ident, seg, (T)0.02);
	});

	snprintf(name, sizeof(name), "  same body as a QUERY (margin 0, no patch)");
	hop::bench::go(name, 200000, [&] {
		c.reset(); c.time = hop::scalar_traits<T>::one();
		seg.set_start_dir(vec(0, 0.3, 0), fall);
	...
	snprintf(name, sizeof(name), "  non-rotating body, discovery margin (no patch)");
	hop::bench::go(name, 200000, [&] {
		c.reset(); c.time = hop::scalar_traits<T>::one();
		seg.set_start_dir(vec(0, 0.3, 0), fall);
		t->trace_solid(c, gib.get(), V {}, ident, seg, (T)0.02);
	});

	// Oriented expansion: the same sweep for a box turned off-axis, so every plane
	// takes the OBB support instead of the per-axis lookup. The axial fast path is
	// preserved by construction (an axial plane's support is precomputed per trace),
	// so the delta here is the oblique planes and the per-trace setup only.
	auto turned = make_spinner(0.2, 0.05, 0.2);
	{
		hop::mat3<T> m;
	...
	snprintf(name, sizeof(name), "  + patch: swept, sliding on floor");
	hop::bench::go(name, 200000, [&] {
		c.reset(); c.time = hop::scalar_traits<T>::one();
		seg.set_start_dir(vec(0, 0.05, 0), slide);
		t->trace_solid(c, plate.get(), V {}, ident, seg, T {});
	});
}
